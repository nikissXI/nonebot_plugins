...
class Translator:
    ...
    def __init__(
        ...
        if proxies:
            self.client = httpx.AsyncClient(
                http2=http2, timeout=timeout, proxies=proxies
            )
        else:
    ...
    async def translate(self, text: str, dest="zh-cn", src="auto"):
        ...
        origin = text
        data, response = await self._translate(text, str(dest), str(src))
        ...
        if isinstance(text, list):
            ...
            for item in text:
                translated = self.translate_legacy(
                    item, dest=str(dest), src=str(src), **kwargs
                )
                result.append(translated)
            return result

        origin = text
        data, response = self.translate_legacy(text, str(dest), str(src))
