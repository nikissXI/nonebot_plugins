...
class TokenAcquirer:
    ...
    async def _update(self):
        ...
        try:
            result_groups = self.RE_TKK.search(r.text)
            if not result_groups:
                raise Exception("result_groups do not have group(1)")
            # this will be the same as python code after stripping out a reserved word 'var'
            code = result_groups.group(1).replace("var ", "")
